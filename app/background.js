/**
 * PSP Detector Chrome Extension Background Script
 * Handles tab events, PSP detection, icon management, and domain exemption logic.
 * @file background.js
 */
...
// Limit for script injection retries
const MAX_SCRIPT_RETRIES = 2;

/**
 * Executes the content script in the given tab, with retry and debounce.
 * @param {number} tabId - The tab ID to inject into.
 * @param {number} [retryCount=0] - Current retry count.
 */
const debouncedExecuteContentScript = debounce((tabId, retryCount = 0) => {
          ...
          results => {
            if (chrome.runtime.lastError) {
              ...
              if (retryCount < MAX_SCRIPT_RETRIES) {
                console.error(
                  `Script injection error on tab ${tabId} (retry ${
                    retryCount + 1
                  }):`,
                  errMsg
                );
                setTimeout(() => {
                  debouncedExecuteContentScript(tabId, retryCount + 1);
                }, 2000);
              } else {
                console.error(
                  `Max retries reached for tab ${tabId}. Injection failed.`
                );
              }
            }
          }
  ...
  if (message.action === 'getExemptDomainsRegex') {
    if (!exemptDomainsRegex) {
      loadExemptDomains().then(() => {
        sendResponse({
          regex: exemptDomainsRegex ? exemptDomainsRegex.source : null
        });
      });
  ...
  try {
    ...
    if (detectedPsp) {
      ...
      if (
        ...
        tab.url &&
        exemptDomainsRegex &&
        exemptDomainsRegex.test(tab.url)
      ) {
        debouncedExecuteContentScript(currentTabId);
      }
    }
  } catch (error) {
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (
    ...
    if (exemptDomainsRegex.test(tab.url)) {
      debouncedExecuteContentScript(tabId);
    } else {
      resetExtensionIcon();
    }
  }
                  ...
                  if (stillExists) {
                    chrome.scripting.executeScript(
                      ...
                      retryResults => {
                        if (chrome.runtime.lastError) {
                          console.error(
                            `Retry injection failed for tab ${tabId}:`,
                            chrome.runtime.lastError.message
                          );
                        }
                      }
...
function applyPspIcon (pspConfig) {
  ...
  if (psp && psp.image) {
    ...
    chrome.action.setTitle({
      title:
        'PSP Detector determined this website is using ' +
        detectedPspName +
        '. Click for more.'
    });
  } else {
