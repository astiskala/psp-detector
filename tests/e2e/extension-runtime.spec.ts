...
async function expectHistoryChartsAndColumns(
  ...
  await expect(
    optionsPage.getByRole('columnheader', { name: 'Type' }),
  ).toBeVisible();
  await expect(
...
test('popup renders seeded detections and keeps button sizing consistent', async ({
  page: _page,
}, testInfo) => {
  ...
  try {
    ...
    const merchantTabId = await createMerchantTab(popupPage, 'popup-seeded');
    ...
    await expect(popupPage.locator('#psp-name')).toHaveText('2 PSPs detected');

    await expect(popupPage.locator('.psp-card')).toHaveCount(2);
    await expect(
      popupPage.getByText('Detection Source:').first(),
    ).toBeVisible();
...
test('popup restores detected PSPs when current tab switches back', async ({
  page: _page,
}, testInfo) => {
  const context = await launchExtensionContext(
  ...
  try {
    ...
    const detectedTabId = await createMerchantTab(popupPage, 'switch-detected');
    ...
    await popupPage.reload({ waitUntil: 'domcontentloaded' });
    await expect(popupPage.locator('#psp-name')).toHaveText('No PSP detected');
...
test('history page opened from popup shows charts and detection metadata columns', async ({
  ...
  try {
    ...
    const merchantTabId = await createMerchantTab(popupPage, 'history-seeded');
    await seedDetectionsForTab(popupPage, merchantTabId);
    await expect.poll(async () => getPspCount(popupPage)).toBe(2);
    await popupPage.reload({ waitUntil: 'domcontentloaded' });
...
test('performance budget: popup renders seeded detections quickly', async ({
  page: _page,
}, testInfo) => {
  const context = await launchExtensionContext(
  ...
  try {
    ...
    console.info(`[perf] popup render with seeded detections: ${elapsedMs}ms`);
